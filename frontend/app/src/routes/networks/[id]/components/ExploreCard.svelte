<script lang="ts">
	...
	let { card, networkId, onedit, onremove, onfullscreen }: Props = $props();
	...
	async function loadAndRender() {
		...
		try {
			const spec = await plots.generateExplore(networkId, {
				bus_carrier: card.bus_carrier,
				query: card.query,
				branch_components: card.branch_components,
			...
			console.error('ExploreCard error:', err);
	...
	let prevKey = '';
	$effect(() => {
		const key = JSON.stringify([card.bus_carrier, card.query, card.branch_components, card.geometry, networkId]);
		if (key !== prevKey) {
			prevKey = key;
			loadAndRender();
		}
...
<div class="group/card bg-card rounded-lg border border-border overflow-hidden flex flex-col h-full relative">
	...
	<div class="card-drag-handle flex items-center justify-between px-3 py-1 border-b border-border/50 shrink-0 cursor-grab">
		{#if onfullscreen}
			<button class="text-xs font-medium text-muted-foreground truncate hover:text-foreground transition-colors cursor-pointer" onclick={onfullscreen}>{card.name ?? 'Explore'}</button>
		{:else}
			<span class="text-xs font-medium text-muted-foreground truncate">{card.name ?? 'Explore'}</span>
		{/if}
